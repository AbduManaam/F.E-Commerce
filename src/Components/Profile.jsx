...
const Profile = () => {
  ...
  // ─── API CALLS — using apiService methods + apiService.client for missing ones ───
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      // Uses apiService.client (the raw axios instance inside ApiService)
  ...
  const handlePasswordChange = async (e) => {
    ...
    // uses the existing changePassword method in ApiService
...
const handleAddressSubmit = async (e) => {
  ...
  if (!/^\d{10}$/.test(addressForm.phone.trim())) { showMessage("error", "Phone must be 10 digits"); return; }
  ...
  if (!/^\d{4,10}$/.test(addressForm.zip_code.trim())) { showMessage("error", "Enter a valid ZIP code"); return; }
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          <div className="flex flex-col md:flex-row items-center md:items-end gap-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => { setActiveTab("overview"); setIsEditing(true); }} className="flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg font-medium transition-colors">
                <Edit3 className="w-4 h-4" /> Edit Profile
              </button>
              <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg font-medium transition-colors">
                ...
                {[
                  ...
                  <button key={item.id} onClick={() => setActiveTab(item.id)}
                    className={`flex items-center gap-3 px-6 py-4 text-left font-medium transition-colors ${
                      activeTab === item.id ? "bg-amber-50 text-amber-700 border-l-4 border-amber-500" : "text-gray-600 hover:bg-gray-50 border-l-4 border-transparent"
                    }`}
                  >
          ...
          <div className="lg:col-span-3">
            ...
            {activeTab === "overview" && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  {!isEditing && <button onClick={() => setIsEditing(true)} className="text-amber-600 hover:text-amber-700 font-medium text-sm">Edit</button>}
                </div>
                {isEditing ? (
                  <form onSubmit={handleProfileUpdate} className="space-y-4 max-w-md">
                    <div>
                      ...
                      <input type="text" value={profileForm.name} onChange={(e) => setProfileForm({ name: e.target.value })}
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent" placeholder="Your name" />
                    </div>
                    <div>
            ...
            {activeTab === "orders" && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                {ordersLoading ? (
                  <div className="flex justify-center py-12"><div className="animate-spin rounded-full h-10 w-10 border-4 border-amber-500 border-t-transparent"></div></div>
                ) : orders.length === 0 ? (
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {(order.Status || order.status) === "pending" && (
                              <button onClick={() => handleCancelOrder(order.ID || order.id)} className="text-xs text-red-600 hover:text-red-700 font-medium border border-red-200 px-2 py-1 rounded-lg">Cancel</button>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            {activeTab === "wishlist" && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <Link to="/wishlist" className="text-amber-600 hover:text-amber-700 font-medium flex items-center gap-1 text-sm">View All <ChevronRight className="w-4 h-4" /></Link>
                </div>
                <div className="text-center py-12 text-gray-500">
                  <Heart className="w-16 h-16 mx-auto mb-4 text-gray-300" />
                  <p className="font-medium">Your saved items are on the Wishlist page</p>
                  <Link to="/wishlist" className="mt-4 inline-block px-6 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-medium transition-colors">Go to Wishlist</Link>
                </div>
              </div>
            )}
            ...
            {activeTab === "addresses" && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  {!showAddressForm && (
                    <button onClick={() => { resetAddressForm(); setShowAddressForm(true); }} className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-medium transition-colors text-sm">
                ...
                {showAddressForm && (
                  <form onSubmit={handleAddressSubmit} className="mb-6 p-4 border border-amber-200 rounded-xl bg-amber-50 space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {[
                        ...
                        <div key={field.key}>
                          ...
                          <input type={field.type} value={addressForm[field.key]}
                    ...
                    <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                      <input type="checkbox" checked={addressForm.is_default} onChange={(e) => setAddressForm({ ...addressForm, is_default: e.target.checked })} className="w-4 h-4 accent-amber-500" />
                      Set as default address
                    </label>
                    <div className="flex gap-3 pt-2">
                      <button type="submit" className="px-5 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-medium text-sm transition-colors">{editingAddress ? "Update" : "Save"} Address</button>
                      <button type="button" onClick={resetAddressForm} className="px-5 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg font-medium text-sm transition-colors">Cancel</button>
                    </div>
                  </form>
                )}
                {addressesLoading ? (
                  ...
                  <div className="text-center py-12 text-gray-500"><MapPin className="w-16 h-16 mx-auto mb-4 text-gray-300" /><p>No addresses saved yet</p></div>
                ) : (
                    ...
                    {addresses.map((addr) => {
                      ...
                      return (
                        <div key={id} className={`rounded-xl p-4 border-2 relative ${isDefault ? "border-amber-500 bg-amber-50" : "border-gray-200 bg-white"}`}>
                          {isDefault && (
                            <span className="absolute top-2 right-2 text-xs font-medium text-amber-700 bg-amber-100 px-2 py-1 rounded-full flex items-center gap-1"><Star className="w-3 h-3" /> Default</span>
                          )}
                          ...
                          <p className="text-sm text-gray-600 mb-3">
                            {addr.Address || addr.address}, {addr.City || addr.city}, {addr.State || addr.state} - {addr.ZipCode || addr.zip_code}, {addr.Country || addr.country}
                          ...
                          <div className="flex gap-3 flex-wrap">
                            ...
                            <button onClick={() => handleDeleteAddress(id)} className="text-sm text-red-600 hover:text-red-700 font-medium flex items-center gap-1"><Trash2 className="w-3 h-3" /> Delete</button>
                            {!isDefault && <button onClick={() => handleSetDefault(id)} className="text-sm text-blue-600 hover:text-blue-700 font-medium">Set Default</button>}
                          </div>
                        </div>
                      );
                    })}
                ...
                <form onSubmit={handlePasswordChange} className="space-y-4 max-w-md">
                  {[
                    ...
                    <div key={item.field} className="relative">
                      <input type={showPassword[item.field] ? "text" : "password"} placeholder={item.placeholder} value={passwordForm[item.key]}
                      ...
                      <button type="button" onClick={() => togglePasswordVisibility(item.field)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                        {showPassword[item.field] ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                      </button>
                    </div>
                  ))}
                  <button type="submit" className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-medium transition-colors">Update Password</button>
                </form>
              </div>
            )}
          </div>
